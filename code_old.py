...
data = np.genfromtxt(fname=file_path, delimiter=',', skip_header=1, dtype=str)
...
bb_temp_K = bb_temp + 273.15
tleft_in_K = tleft_in + 273.15
tright_in_K = tright_in + 273.15
tleft_out_K = tleft_out + 273.15
...
# Mittelwerte
11
avg_bb_temps, averages_left_in, averages_right_in, averages_left_right_in, averages_out = [], [], [], [], []
for ts in time_stamps:
    idx = np.argmin(np.abs(np.array(time_dt) - ts))
    start = max(0, idx - 19)
    end = idx + 1
    avg_bb_temps.append(np.mean(bb_temp_K[start:end]))
    averages_left_in.append(np.mean(tleft_in_K[start:end]))
    averages_right_in.append(np.mean(tright_in_K[start:end]))
    averages_left_right_in.append(np.mean((tleft_in_K[start:end] + tright_in_K[start:end]) / 2))
    averages_out.append(np.mean(tleft_out_K[start:end]))
avg_bb_temps = np.array(avg_bb_temps)
averages_left_right_in = np.array(averages_left_right_in)
print("T_bb")
print(avg_bb_temps)
print("T_Umgebung")
print(averages_left_right_in)
# Statistische Fehler: Standardabweichung / sqrt(n)
n_points = 20
bb_std_err, env_std_err = [], []
for ts in time_stamps:
    idx = np.argmin(np.abs(np.array(time_dt) - ts))
    start = max(0, idx - (n_points - 1))
    end = idx + 1
    # Schwarzkörper
    bb_slice = bb_temp_K[start:end]
    std_bb = np.std(bb_slice, ddof=1)
    bb_std_err.append(std_bb / np.sqrt(n_points))
    # Umgebung (Mittel aus links + rechts)
    env_slice = (tleft_in_K[start:end] + tright_in_K[start:end]) / 2
    std_env = np.std(env_slice, ddof=1)
    env_std_err.append(std_env / np.sqrt(n_points))

bb_std_err = np.array(bb_std_err)
env_std_err = np.array(env_std_err)
# Verlustleistung
delta_T = avg_bb_temps - averages_left_right_in
P_loss = (
...
plt.tight_layout()
plt.show()
# Plot: Berechnete über T_BB
plt.figure(figsize=(10, 6))
plt.errorbar(avg_bb_temps, sigma_calc, xerr=t_error, yerr=sigma_stat_error, fmt='o', label='Stat. Fehler', color='red', capsize=3)
plt.errorbar(avg_bb_temps, sigma_calc, xerr=t_error, yerr=sigma_error, fmt='o', label='System. Fehler', color='black', capsize=3)
...
plt.figure(figsize=(10, 6))
plt.errorbar(avg_bb_temps, epsilon, xerr=t_error, yerr=epsilon_stat_error, fmt='o', label='Stat. Fehler', color='red', capsize=3)
plt.errorbar(avg_bb_temps, epsilon, xerr=t_error, yerr=epsilon_error, fmt='o', label='System. Fehler', color='black', capsize=3)
plt.xlabel("Schwarzkörpertemperatur (K)")
